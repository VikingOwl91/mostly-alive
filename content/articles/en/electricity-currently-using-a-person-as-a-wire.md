...
### The Physics of Tetanic Muscle Clamping ("Let-Go Threshold")

Household alternating current triggers repetitive muscle contractions at 50 to 60 cycles per second. Because the human forearm flexor muscles (which close the fingers into a fist) are significantly stronger than the extensor muscles (which open the hand), current entering through the palm forces an involuntary, unbreakable grip on the energized wire.

This is the **"can't let-go" threshold** (typically occurring at just 10 to 16 milliamperes). If a bystander grabs the victim with bare hands, the current instantly branches into the rescuer's palm, clamping both individuals into the lethal circuit.

### Low-Voltage vs. High-Voltage Physics

1. **Low-Voltage (< 1,000 Volts / Domestic)**:
   - Current requires direct physical contact with conductive elements.
   - Primary fatal mechanism is cardiac ventricular fibrillation and respiratory muscle tetany.
   - Rescuer priority: Disconnect the main breaker switch or separate using dry insulating materials (wood, plastic).
